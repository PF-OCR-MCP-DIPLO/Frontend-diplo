...
import type { ResultsValidationMap, FieldValidationIssue } from '@/features/processing/components/results/results-validation';
...
interface ResultsFieldDetailPanelProps {
  open: boolean;
  jobId: number;
...
export function ResultsFieldDetailPanel({ open, jobId, row, field, validationMap, reprocessingDepositId, onClose, onEdit, onAskAssistant, onReprocessDeposit }: ResultsFieldDetailPanelProps) {
    ...
    <aside className='absolute inset-y-0 right-0 z-20 w-full max-w-[400px] border-l border-border/70 bg-background/96 shadow-[var(--shadow-floating)] backdrop-blur lg:absolute'>
      <div className='flex items-center justify-between border-b border-border/60 px-4 py-3'>
        <div>
          ...
          <p className='text-xs text-muted-foreground'>{getFieldLabel(field)} · Fila {row.referencia || row.id}</p>
        </div>
