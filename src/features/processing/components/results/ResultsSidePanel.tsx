...
function resolveTitle(
  ...
  if (mode === "field-detail")
    return detailCell
      ? `Detalle de ${getFieldLabel(detailCell.field)}`
      : "Detalle del campo";
  return "Preview";
}
...
export function ResultsSidePanel({
  ...
  const detailRow = panelState.detailCell
    ? (data.find((row) => row.id === panelState.detailCell?.rowId) ?? null)
    : null;
    ...
    >
      {panelState.mode === "issues" || panelState.mode === "error" ? (
        <div className="space-y-3 text-sm">
          {errorMessage ? (
            <p className="rounded-xl border border-border/60 bg-surface-subtle p-3 text-foreground">
              {errorMessage}
            </p>
          ) : null}
          <ResultsErrorPanel
      ...
      {panelState.mode === "logs" ? (
        <div className="space-y-3 text-sm">
          ...
          {logs.map((log) => (
            <div
              key={log.id}
              className="rounded-xl border border-border/60 bg-surface-subtle p-3"
            >
              <p className="font-medium text-foreground">
                #{log.sequence_index} · {log.stage}
              </p>
      ...
      {panelState.mode === "preview" ? (
        <ResultsPreviewPanel
          fileName={
            sourceDocxUrl
              ? (sourceDocxUrl.split("/").pop() ?? "Documento")
              : "Documento fuente"
          }
