...
export function DocumentPreview({
  ...
  useEffect(() => {
    ...
    const nextIndex = images.findIndex((image) => image.id === selectedImageId);
  ...
  const currentImageIssues = currentImage
    ? (validationMap.imageIssuesById[currentImage.id] ?? [])
    : [];
  const currentImageStatus = currentImage
    ? getImageStatus(currentImage.id, validationMap)
    : "neutral";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Compara el resultado con el documento fuente.
            </p>
          </div>
        </div>
