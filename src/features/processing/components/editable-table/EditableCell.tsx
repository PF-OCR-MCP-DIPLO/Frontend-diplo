...
import type {
  ...
  ResultsValidationMap,
} from "@/features/processing/components/results/results-validation";
import {
  getCellIssueSummary,
  getCellStatus,
  getFieldLabel,
...
export function EditableCell({
  ...
  const pillClass = `block w-full max-w-full truncate rounded-md px-2 py-1 text-sm transition ${hasError ? "bg-danger/8 text-danger ring-1 ring-danger/20" : cellStatus === "valid" ? "bg-success/5 text-foreground ring-1 ring-success/20" : "text-surface-foreground"} ${isSelected ? "ring-1 ring-primary/25" : ""}`;
  if (isEditing) {
    return (
      <Input
        autoFocus
        ...
        content={
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              Valor actual: {tooltipModel.currentValue}
            </p>
            <p className="text-muted-foreground">
              Sugerencia: {tooltipModel.correctionHint}
            </p>
          </div>
        }
      />
